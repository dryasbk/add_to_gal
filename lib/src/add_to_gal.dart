import 'dart:async';
import 'dart:io';
import 'package:add_to_gal/src/copy_to_permenant_directory.dart';
import 'package:add_to_gal/src/get_file_type.dart';
import 'package:flutter/services.dart';

/// Save images and videos to the gallery
class AddToGal {
  static const MethodChannel _channel = MethodChannel('add_to_gal');

  /// Makes a COPY of the file and saves it to the gallery
  ///
  /// Returns the path of the new file in the gallery
  static Future<File> addToGal({
    ...
    if (Platform.isIOS) {
      ...
      if (filetype == 'image') {
        methodResults = await _channel.invokeMethod('putImage', {
          'path': copiedFile.path,
          'album': albumName,
        });
        ...
        methodResults = await _channel.invokeMethod('putVideo', {
          'path': copiedFile.path,
          'album': albumName,
        });
        //   'saveVideo',
